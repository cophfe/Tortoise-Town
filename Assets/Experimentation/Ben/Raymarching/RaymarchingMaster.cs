...
public class RaymarchingMaster : MonoBehaviour
{
    ...
    void CreateScene()
    {
        List<RaymarchingShape> allShapes = new List<RaymarchingShape>(FindObjectsOfType<RaymarchingShape>());
        allShapes.Sort((a, b) => a.operation.CompareTo(b.operation));

        List<RaymarchingShape> orderedShapes = new List<RaymarchingShape>();
        ...
        for (int i = 0; i < allShapes.Count; i++)
        {
            ...
            if (allShapes[i].transform.parent == null)
            {
                ...
                {
                    if (parentShape.GetChild(j).GetComponent<RaymarchingShape>() != null)
                    {
                        orderedShapes.Add(parentShape.GetChild(j).GetComponent<RaymarchingShape>());
