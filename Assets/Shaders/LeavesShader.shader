...
Shader "LeavesShader"
{
	Properties
	{
		...
		[ASEEnd]_AlphaClipThreshold("AlphaClipThreshold", Range( 0 , 1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float4 _Albedo_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(LeavesShader,_Albedo_ST);
				float2 uv_Albedo = IN.ase_texcoord7.xy * _Albedo_ST_Instance.xy + _Albedo_ST_Instance.zw;
				float4 tex2DNode6 = tex2D( _Albedo, uv_Albedo );
				
				float _AlphaClipThreshold_Instance = UNITY_ACCESS_INSTANCED_PROP(LeavesShader,_AlphaClipThreshold);
